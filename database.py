...
def insert(nom, mail, tel, adresse):
    cur.execute("insert into listcontacts (Nom, Email, NumTel, Adresse) values (?, ?, ?, ?)",
                (nom, mail, tel,  adresse))
    connexion.commit()
...
def updatemail(mail, id):
    cur.execute(
        "update listcontacts set Email = '%s' where Id= '%s' " % (mail, id))
    connexion.commit()


def updatetel(tel, id):
    cur.execute(
        "update listcontacts set NumTel = '%s' where Id= '%s' " % (tel, id))
    connexion.commit()


def updatenom(adr, id):
    cur.execute(
        "update listcontacts set Adresse = '%s' where Id= '%s' " % (adr, id))
    connexion.commit()
